...
**当前保留 5 个业务接口**（6 对 request/response 样本，`user/portrait` 有两套参数样本）：
...
| trend/chart | `/api/v1/insights/workbench/live/detail/trend/chart` | P1 | ✅ **已补充** |
...
**覆盖度**: 核心数据需求（GMV、流量、转化、商品、观众画像、趋势对比）
...
├── API-inventory.md       ← API 详细目录（5 个业务接口字段表 + 实现优先级）
├── summary.md             ← workflow 生成的完整设计文档（备查）
└── raw/                   ← API 样本文件（6 对 request + response，含 2 套 user/portrait）
    ├── 03-source-new.*
    ├── 04-user-portrait.*
    ├── 04b-user-portrait.*
    ├── 05-product-list.*  ← 新增（2026-06-11 补充抓包）
    ├── 06-core-stats.*
    └── 08-trend-chart.*   ← 新增（2026-06-11 补充抓包）
```
...
| 任务 | 估时 | 产出 |
|------|------|------|
| 1. 在 live-platform 新增 `DashboardCollector` 类 | 1.5d | 封装 `fetch_core_stats` + `fetch_product_list` |
...
2. **覆盖范围从基础指标扩展到核心大屏接口**
   - 初始 5 个 API → 当前保留 5 个业务接口（6 对样本，含商品列表 + 性能趋势图 + 两套画像参数）
   - P0 API 完整（core_stats + product_list）

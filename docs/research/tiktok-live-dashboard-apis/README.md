...
| **room/info** | 房间与主播信息（主播名/头像/标题/封面/开播时间/直播状态/FLV 地址） | ✅ 完整 | **P0** |
| **trend/chart** | 性能趋势曲线（Viewers + GMV 时间序列），支持 Full LIVE / Last 5m / Last 30m 三种时间范围 | ✅ 完整 | **P1** |
...
**结论**：P0 API（`core/stats` + `product/list` + `room/info`）可支撑 MVP 上线，覆盖核心指标、商品明细与大屏头部展示。

...
from orchestrator.seed_synchronizer import seed_synchronizer
...
async def lifespan(app: FastAPI):
    ...
    # 初始化 MySQL 连接池（对齐 live-monitor 口径）
    ...
    # 首次全量同步种子（启动时执行一次）
    logger.info("执行首次种子同步（MySQL → Redis）...")
    result = await seed_synchronizer.sync_once()
    logger.info(
        f"首次种子同步完成 | total={result['total']} "
    ...
    # 种子同步任务（对齐 live-monitor 5 分钟周期）
    scheduler.add_job(
        seed_synchronizer.sync_once,
        "interval",
        seconds=settings.mysql.sync_interval_seconds,
        id="seed_sync",
    )
    scheduler.start()

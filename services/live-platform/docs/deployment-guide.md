...
4. 开播时，如果活跃录制数小于 `cutliveNumber`，启动 MediaMTX path 和 FFmpeg relay。
5. 每轮检测都会写入 `live:collection:{collectionId}:status`。
6. MediaMTX 完成切片后调用 `/internal/segment-ready`。
7. Upload worker 先上传 OSS,再推送 Kafka(payload 含 `dataSource: live_crawler_{platform}`)。
...
3. 如果已启动 FFmpeg 录制进程，回滚前确认旧 Pod 已停止，避免同一 collectionId 被重复录制。

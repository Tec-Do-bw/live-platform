...
class Downloader:
    ...
    def run(self, tasks: list[Task | str]) -> list[DownloadResult]:
        ...
        logger.info(f"开始采集，共 {total} 个任务，并发 {self._workers}" )
        ...
        with ThreadPoolExecutor(max_workers=self._workers) as pool:
            ...
            for future in as_completed(future_to_idx):
                ...
                if done_count % max(1, total // 10) == 0 or done_count == total:
                    logger.debug(f"进度: {done_count}/{total} ({done_count / total * 100:.0f}%)")


        self._total_elapsed = (time.monotonic() - start_time) * 1000
        self._results = results
        logger.info(f"采集完成，耗时 {self._total_elapsed / 1000:.1f}s")
        return results

...
class WebSocketClient:
    def __init__(self, user_id: str,url:str='ws://47.236.42.104:8081/ws/'):
    ...
    async def send_message(self, target_user_id: str, message: str):
        ...
        try:
            ...
            print(f"消息已发送给用户 {target_user_id}: {message_data}")
        except Exception as e:
            print(f"发送消息失败: {e}")
            self.connected = False
...
#"""Sqlserver"""
class DataBaseSqlserver(BaseDBClass):
    def __init__(self, conn_params):
        self.pool = PooledDB(pyodbc,maxconnections = 50,mincached=5,maxcached=20, driver='{SQL Server}', host=conn_params['host'], database=conn_params['dbname'], user=conn_params['user'], password=conn_params['password'])
        self.db = conn_params['dbname']
    def help(self):
        return "这是SQLServer数据库"
    
    def createDB(self,dbname):
        try:
            conn = self.pool.connection()
            cursor = conn.cursor()
            sql = 'IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = N\'{}\')  BEGIN  CREATE DATABASE {};  ALTER DATABASE {} SET RECOVERY SIMPLE;  END'.format(str(dbname),str(dbname),str(dbname))
            cursor.execute(sql)
            conn.commit()
            cursor.close()
            conn.close()
    ...
    def createTB(self,tablename,fieldDict):
        try:
            ...
            for column in fieldDict.items():
                create_column += (str(column[0]) +' '+str(column[1]) +' default NULL,')
            sqlTmp = 'create table  {} ({}) '.format(tablename,create_column[:-1])+';'
            sql = "IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = \'{}\')".format(str(tablename)) + "BEGIN {} END".format(str(sqlTmp))
            cursor.execute(sql)
            conn.commit()
        ...
        try:
            for data in datalist:
                ...
                for column in [column for column in data.keys()]:
                    columns += (column+',')
                    value += ('\''+data[column]+'\''+',')
                sql = 'insert into {}({}) values({}) '.format(tablename,str(columns[:-1]),str(value[:-1]))+';'
    ...
    #查询数据(默认返回[set,set,set]格式,可选参数["set","json"])
    def selectData(self,tablename,columnList,returnType="set"):
        conn = self.pool.connection()
        cursor = conn.cursor()
        try:
            data_columns = ''
            ...
            for row in rows:
                ...
                for r in row:
                    if r is None:
                        r = "NULL"
                    elif "int" in str(type(r)):
                        r =  r
                    else:
                        r = r.strip()
                    tmprList.append(r)
                column_all.append(tmprList)
            cursor.close()
            conn.close()
            if returnType=="set":
                return column_all
            elif returnType=="json":
                column_all_json = list()
                for tmp_i in column_all:
                    tmp_dict = {}
                    for tmpIndex in range(len(tmp_i)):
                        tmp_dict[columnList[tmpIndex]] = tmp_i[tmpIndex]
                    column_all_json.append(tmp_dict)
                return column_all_json
            else:
                return column_all
        except Exception as e:
            print("查找表 %s 时,出现错误 %s",tablename,e)
    ...
    def tableExists(self,table_name,copyTable=""):
        ...
        query = f"""
            ...
            AND TABLE_NAME = '{table_name_tmp}'
            """
        cursor.execute(query)
        result = cursor.fetchone()[0]
        if result <= 0:
            CreateSql =  "select * into {} from {} where 1>2".format(str(table_name),str(copyTable))
            self.exeSql(CreateSql)
    ...
    def exeSql(self,sql):
        try:
            conn = self.pool.connection()
            cursor = conn.cursor()   
            if str(sql).strip().lower()[:6] == 'select' and 'into' not in sql:
                cursor.execute(sql)
                rows = cursor.fetchall()
                results = []
                for row in rows:
                    # results.append(['NULL' if r is None else r.strip() for r in row])
                    tmprList = []
                    for r in row:
                        if r is None:
                            r = "NULL"
...
#"""PostgreSQL"""
class DataBasePostgreSQL(BaseDBClass):
    def __init__(self, conn_params):
        self.pool = PooledDB(psycopg2,maxconnections = 50,mincached=5,maxcached=20,  host=conn_params["host"], dbname=conn_params["dbname"], user=conn_params["user"], password=conn_params["password"])
        self.db = conn_params['dbname']
        self.conn_params = conn_params
    def help(self):
        return "这是PostgreSQL数据库"

    def createDB(self,dbname):
        try:
            #'在 PostgreSQL 中，CREATE DATABASE 语句不能在一个事务块内运行,所以链接池模式不支持该创建数据库,这里改为临时链接创建'
            ...
            cur = conn.cursor()
            # 执行创建数据库的 SQL 语句
            cur.execute("CREATE DATABASE {}".format(str(dbname)))
            # 关闭游标
            cur.close()
            # 关闭连接
            conn.close()
    ...
    def createTB(self,tablename,fieldDict):
        try:
            ...
            for column in fieldDict.items():
                create_column += (str(column[0]) +' '+str(column[1]) +' default NULL,')
            sql = 'create table  {} ({}) '.format(tablename,create_column[:-1])+';'
            cursor.execute(sql)
            conn.commit()
...
class dataBase:
    ...
    def connect(conn_params={}):
        ...
        elif conn_params["DBType"]  == "Sqlserver":
            return DataBaseSqlserver(conn_params)
        elif conn_params["DBType"]  == "Postgresql":
            return DataBasePostgreSQL(conn_params)
...
# ✅华为云OBS相关操作
class HwyOBSHelper:
    def __init__(self,ak='6IJ3QXNPG6T21FMOETOG',sk='CexI7LB9m83X0kiIYoP75qmvXMXFWfwtsPCXQ8V4',server='https://obs.ap-southeast-3.myhuaweicloud.com',bucketName='hwsg-mth-data-insight-prod-01',endpoint="https://hwsg-mth-data-insight-prod-01.obs.ap-southeast-3.myhuaweicloud.com"):
        self.ak = ak
        self.sk = sk
        self.server = server
        self.bucketName = bucketName
        self.endpoint = endpoint  # 存储endpoint参数，用于自定义访问域名或API网关地址
        self.obs_client = ObsClient(access_key_id=self.ak,secret_access_key=self.sk,server=self.server)
    #文件上传OBS,传入文件路径与对象key
    def upload_file(self,file_path,object_key):
        try:
            resp = self.obs_client.putFile(bucketName=self.bucketName, objectKey=object_key, file_path=file_path)
            if resp.status < 300:
                # 获取对象的URL
                signed_url = self.obs_client.createSignedUrl(
                    ...
                    expires=8640000  # URL有效期，单位为秒 
                )
                # 可以使用endpoint替换默认的OBS域名，提供自定义域名访问
                file_url = signed_url["signedUrl"]
                if self.endpoint:
                    # 如果提供了endpoint，可以用它替换原始URL中的域名部分
                    parsed_url = urlparse(file_url)
                    file_url = file_url.replace(f"{parsed_url.scheme}://{parsed_url.netloc}", self.endpoint)
                
                returnData = {"code": 200, "fileUrl": file_url, "message": "上传成功"}
                return returnData
            else:
                returnData = {"code": resp.errorCode, "fileUrl": None,"file_path":file_path,"message": resp.errorMessage}
                return returnData
        except Exception as e:
            print('发生异常:', e)
...
#启动浏览器多线程采集
class ChomeCrowl():
    #浏览器GPU渲染gpuFlag开关1==有头；0==无头
    def __init__(self,SpiderConfig={"gpuFlag":1,"curlDataThreadnum":1}):
        self.Threadnum = SpiderConfig['curlDataThreadnum']
        self.gpuFlag = SpiderConfig['gpuFlag']
        # 给浏览器开始flash选项
        self.prefs = {
        ...
        if len(proxyList)>0:
            ...
            self.chromeOpitons.add_argument(f'--proxy-server={proxy}')
                # 浏览器配置信息对象增加flash选项
        self.chromeOpitons.add_experimental_option('prefs', self.prefs)
                # 关闭浏览器自动化测试控制提示
        self.chromeOpitons.add_experimental_option("useAutomationExtension", False)
        self.chromeOpitons.add_experimental_option("excludeSwitches", ['enable-automation'])
        # self.chromeOpitons.add_experimental_option('excludeSwitches', ['enable-logging'])
        self.chromeOpitons.add_argument('--disable-blink-features=AutomationControlled')
        # 是否无头执行，默认有头界面
        if self.gpuFlag  == 0:
            self.chromeOpitons.add_argument("--headless")#无头
            self.chromeOpitons.add_argument("--disbale-gpu")#无gpu图形化界面
        self.driver = webdriver.Chrome(options=self.chromeOpitons)  # 爬虫浏览器
        # 设置页面加载超时时间为 10 秒
        self.driver.set_page_load_timeout(30)
        self.driver.maximize_window()
    def help(self):
        helpContent = """
        关于类ChomeCrowl使用用法,ChomeCrowl基于selenium库,参考格式如下写法：
        ----------------------------------------------------------------------------------------------
        请求源码方式1:ChomeCrowl代码作为单独模块调用:
            A = ChomeCrowl(gpuFlag,spiderDataQueue)
            A.Run(spiderUrlList=[],Threadnum=10,TmpFunction=None,additionalOperationsRuleList=[])
            关于入参说明：
                gpuFlag:是否使用GPU渲染--->非必填,默认值为1,有头模式
                spiderDataQueue:采集数据队列--->非必填,默认值为{"curlData":"采集URl队列","parseAndCheckData":"解析保存数据队列","saveData":"保存数据队列","errorData":"保存错误数据队列","SpiderConfig":"配置信息队列","functionData":"临时解析保存数据函数队列"}
                spiderUrlList:需要采集的URL列表--->非必填,采集uRL列表，默认留空，当列表不为空的时候，作为单独模块使用
                Threadnum:开启的线程数--->非必填,默认值为10
                TmpFunction:临时解析保存数据函数--->非必填,默认留空，以便作为单独模块使用时,解析保存数据使用
                additionalOperationsRuleList:额外操作，例如：点击关闭页面弹窗，或者验证码点击等等["规则1"，"规则2""]，按照列表顺序点击--->非必填,默认留空
        ----------------------------------------------------------------------------------------------
        """
        print(helpContent)
        return 
    ...
    def open_new_tab(self):
        ...
        for i in range(self.Threadnum):
            ...
            tmpList.append(new_tab_handle)
        return tmpList

    #执行结束关闭所有标签页
    def close_new_tab(self,tmpList):
        for new_tab_handle in tmpList:
            self.driver.switch_to.window(new_tab_handle)
            self.driver.close()


    #传入需要获取源码的网页URl,默认开启10个标签页,
    #这里对spiderUrlList方式进行预留。如果走框架则不需要设置，默认留空，以便单独传入采集列表作为单独模块使用,单独使用需要传入临时解析保存数据函数TmpFunction
    @error_logging_decorator(True)
    def Run(self,q_Url,TmpFunction=None,execFuntion=None):
        flag = 0
        tmpList = self.open_new_tab()
        i = 0
        while not q_Url.empty():
            page_source = ""
            self.driver.switch_to.window(tmpList[i])
            SpiderURL= q_Url.get()
            print("q_Url.qsize()",q_Url.qsize())
            try:
                self.driver.get(SpiderURL)
                if execFuntion:
                    execFuntion(self.driver)
                time.sleep(200)
                # self.driver.set_page_load_timeout(30)
                page_source = self.driver.page_source
                if TmpFunction:
                    TmpFunction(page_source,SpiderURL)
                    continue
                return {'url': SpiderURL, 'html': page_source, 'crawltime': datetime.now()}
            except:
                spiderLogWrite("Page load timed out after 10 seconds.")
            i = 0 if i == (self.Threadnum-1) else i + 1
